...
def prepare_basic_expectations_fields() -> None:
    ...
    input_path = Path(__file__).resolve().parent / "AllFeesGoodForExpectationUpload.json"
    try:
    ...
    for idx, fee in enumerate(all_fees, start=0):
        try:
            ...
            try:
                valuation_f = float(valuation)
            except (TypeError, ValueError):
                skipped_zero_valuation += 1
                print(f"[SKIP] Fee {fee_id}: invalid valuation {valuation!r}")
                continue

            if valuation_f == 0:
                skipped_zero_valuation += 1
                print(f"[SKIP] Fee {fee_id}: plan {plan_id} valuation = 0")
                continue

            percentage = fee.get("Percentage")
            try:
                percentage_f = float(percentage)
            except (TypeError, ValueError):
                skipped_zero_expected_fee += 1
                print(f"[SKIP] Fee {fee_id}: invalid percentage {percentage!r}")
                continue

            expected_fee_amount = percentage_f * valuation_f / 100.0
            if round(expected_fee_amount, 2) == 0:
                skipped_zero_expected_fee += 1
                print(f"[SKIP] Fee {fee_id}: expected fee is 0 (percentage={percentage_f}, valuation={valuation_f})")
                continue
    ...
    output_path = Path(__file__).resolve().parent / "ExpectationsToUpload.json"
    try:
